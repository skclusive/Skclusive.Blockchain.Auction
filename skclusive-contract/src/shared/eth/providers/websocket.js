...
export default class WebsocketProvider extends Web3.providers
  .WebsocketProvider {
  ...
  send(payload, callback) {
    ...
    return this.rewritePayloads(0, requests, {}, () => {
      // @ts-ignore
  ...
  rewritePayloads(index, requests, session_nonces, finished) {
    ...
    this.signer.hasAddress(sender, (err, has_address) => {
          ...
          super.send(
            ...
            function(err, result) {
              if (err != null) {
                ...
                done(null, Web3.utils.toDecimal(new_nonce));
              }
            }
          );
        }
      ...
      getNonce((err, nonce) => {
        ...
        tx_params.nonce = Web3.utils.toHex(final_nonce);
        ...
        this.signer.signTransaction(tx_params, function(err, raw_tx) {
          if (err != null) {
            return next(err);
          }
