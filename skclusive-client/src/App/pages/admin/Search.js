...
class Search extends Component {
  ...
  acceptEther(user, index) {
    ...
    return async () => {
      this.setState({acceptedIndexes: [...this.state.acceptedIndexes, {index, credited: false}]});
      await fetch.postJson(`/api/ether/accpet/request`, data);
      this.setState({acceptedIndexes: [...this.state.acceptedIndexes.slice(0, index-1), {index, credited:true}, ...this.state.acceptedIndexes.slice(index)]});
    };
  }

  searchUser = name => event => {
    console.log("Search change event value :: ", event.target.value);
  ...
  render() {
    const { users, message, status, isLoading, displayUsers, acceptedIndexes } = this.state;
    const { classes } = this.props;
    // console.log(users);
    return (
      <Container className="UserRequest">
        <Auction />
        <React.Fragment>
          <div>
            {status && status == "OK" ? (
